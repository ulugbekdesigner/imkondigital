...
export default async function MentorshipDetailPage({ params }: { params: { id: string } }) {
  ...
  if (!me) redirect(`/kirish?next=/mentorships/${params.id}`);
    ...
    <div className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-2xl px-4 py-10">
        ...
        <p className="mt-3 font-mono text-xs text-ink-soft">
          ...
          {mentorship.responded_at && ` · Javob berilgan: ${formatDate(mentorship.responded_at)}`}
        </p>
        ...
        {!isMentor && mentorship.status === 'pending' && (
          <div className="imk-locked mt-6">
            <LockIcon width={20} height={20} aria-hidden="true" className="shrink-0" />
            <p className="font-sans text-base">
              So'rov {counterpart}ga yuborildi — javob kutilmoqda, check-in hali ochilmagan.
            </p>
          </div>
        )}
          ...
          <GlassCard className="mt-6 p-6">
            <CardHeader>
              ...
              <CardDescription className="flex items-center gap-1.5">
                <LockIcon width={14} height={14} className="shrink-0" aria-hidden="true" />
        ...
        {!isMentor && mentorship.status === 'active' && (
          <div className="mt-6 flex items-start gap-2 rounded-lg border border-line bg-mint/40 px-3 py-2.5">
            <LockIcon width={16} height={16} aria-hidden="true" className="mt-0.5 shrink-0 text-ink-soft" />
            <p className="font-sans text-xs text-ink-soft">
              Yangi check-in yozish faqat ustozga ochiq — siz uni pastdagi ro'yxatda o'qiy olasiz.
            </p>
          </div>
        )}
            ...
            <ul className="flex flex-col gap-2">
              {mentorship.checkins.map((c) => (
                <li key={c.id} className="rounded-lg border border-line px-3 py-2">
                  <p className="font-mono text-xs text-ink-soft">{formatDateTime(c.created_at)}</p>
                  <p className="mt-1 whitespace-pre-wrap font-sans text-base text-ink">{c.note}</p>
                </li>
              ))}
            </ul>
          ) : (
            <div className="imk-empty items-center text-center">
              <span className="imk-empty__icon mx-auto">
                <ClipboardIcon width={22} height={22} aria-hidden="true" />
              </span>
              <h2 className="font-display text-lg font-bold text-ink">Hali check-in yo'q</h2>
